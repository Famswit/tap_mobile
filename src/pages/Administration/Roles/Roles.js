        ...
        <Box sx={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
          {roles.map((role, index) => (
            <Card
              key={index}
              sx={{
                width: '353px', height: '159px',
                borderRadius: '6px', background: 'var(--Colours-White-100, #FFF)',
                boxShadow: '0px 2px 6px 0px rgba(0, 0, 0, 0.10)',
              }}
            >
              <Box sx={{ display: 'flex', padding: '15px', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant='h2' sx={{ fontSize: '16px' }}>{role.name}</Typography>
                <Typography variant='h5' sx={{ fontSize: '9px' }}> Last modified at {role.lastModified}</Typography>
              </Box>
              <Box sx={{ display: 'flex', padding: '34px 15px', justifyContent: 'space-between', alignItems: 'center' }}>
                <Avatar sx={{ bgcolor: "#F0E9E9", width: '32px', height: '32px', marginTop: '25px', fontSize: '.9rem', color: '#8B5C5D' }}>{role.avatar}</Avatar>
